...
// returns zero hits WITHIN themed chip, badge, banner, or stat-row rule
...
// selector identifies it as a themed chip/badge/banner/stat-row -- this
...
// Selector-name keywords identifying a themed chip/badge/banner/stat-row
...
const KEYWORDS = /(chip|badge|banner|stat-row|stat-bar|bar-container|bar-fill|\.status-bar|\battribute-bar\b|\brarity-label\b|\btag\b)/i;
...
console.log(`${scanned} height:<number> declaration(s) matched the AC-038 grep pattern sitewide; ${flaggedBlocks} fall inside a themed chip/badge/banner/stat-row selector block.`);
if (flaggedBlocks === 0) {
  console.log('ok: zero fixed-height declarations inside a themed chip/badge/banner/stat-row rule block (the rest are icon circles, image frames, and progress-bar TRACKS -- non-text containers, where fixed height is correct, not a 1.4.12 risk).');
}

console.log(failures === 0 ? '\nPASS (AC-038, mechanical).' : `\nFAIL: ${failures} failure(s) (AC-038).`);
console.log('Manual follow-up still owed (per VERIFY): the text-spacing bookmarklet pass on the rarity chips and stat rows at a milestone, not blocking CI (research-ui-a11y.md §7.4).');

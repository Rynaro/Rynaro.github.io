...
> **Posture, inherited unchanged from SPECTRA:** an unverified or failed
> envelope is refused and handed back to the orchestrator — never processed
> with a warning. Provenance is only a differentiator if receivers actually
> reject tampered payloads — end to end, not just at the orchestrator.

---

## Where the cryptographic check runs (and why the receiver only reads)

RAMZA's tool surface cannot run the SHA-256 gate itself for INBOUND artefacts
(receiver Eidolons have restricted or no Bash beyond the `ramza-*` gate tools).
The mechanical check therefore runs **at the orchestrator**, once, before RAMZA
is dispatched:
...
`message_id`. RAMZA then enforces the result using **only `Read`** — no Bash
required:
...
**Defense-in-depth (optional):** if RAMZA's host happens to grant Bash with
...
mcp__crystalium__recall(
  scope  = { project: <cwd-project>, agent_class_visibility: "ramza" },
...
## Inbound edges RAMZA accepts

`to.eidolon` MUST equal `ramza`. `from.eidolon` MUST be a declared upstream
sender. Performative and `artifact.kind` are validated against the ECL inbound
contract `contracts/<from>-to-ramza.yaml` (source of truth); the table below is
...
2. Print to stderr: `[ramza-verify-incoming] REFUSE: <FAILURE_CODE> from <from.eidolon>`.
...
**verify_pass:**
```json
{"ts":"<RFC3339>","event":"verify_pass","message_id":"<uuid>","thread_id":"<uuid>","from":"<eidolon>@<version>","to":"ramza@<version>","performative":"<performative>","integrity_method":"sha256"}
```

**verify_fail:**
```json
{"ts":"<RFC3339>","event":"verify_fail","message_id":"<uuid>","thread_id":"<uuid>","from":"<eidolon>@<version>","to":"ramza@<version>","integrity_method":"sha256","verify_failure_code":"<CODE>","decision":"refused"}
```
...
- **Mechanical gate, single source of truth.** The SHA-256 comparison is the
  ...
  LLM-estimated in this skill. RAMZA's own outbound emissions are separately
  self-gated by `ramza-verify-emit` (see above) — the two never substitute for
  each other.
